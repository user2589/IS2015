...
def stats(d):
    ...
    response = urllib.urlopen("http://stats.wikimedia.org/archive/squid_reports"
                              "/{year}-{month:0>2d}/SquidReportClients.htm"
    ...
    for row in rows[2:]:  # first row is table title, second is th
        tds = row.getchildren()
        if len(tds) == 5:
            tds = [tds[0], tds[2], tds[4]]
        if len(tds) != 3 or tds[0].tag != 'td' or tds[0].text == 'Total':
            break  # only non mobile browsers, they come first
        share = float(tds[2].text.strip("%"))
        browser = tds[0].text.replace(",", "_").replace(";", "_")
        assert browser not in res
        res[browser] = share
